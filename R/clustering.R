...
fastcluster <- function( x, k=NULL, rdname="PCA", nstart=3, iter.max=50,
  ...
  x <- bluster::makeKNNGraph(as.matrix(x), k=min(max(2,floor(sqrt(length(unique(k))))-1),10))
  if(returnType=="graph") return(list(k=k, graph=x))
  cl <- igraph::membership(igraph::cluster_louvain(x))
  cl[k]
}
...
.prepSCE <- function(sce, ndims=30, nfeatures=1000, ...){
    ...
    if(!("PCA" %in% reducedDimNames(sce))){
        sce <- scater::runPCA(sce, ncomponents=ifelse(is.null(ndims),30,ndims),
...
.getMetaGraph <- function(x, clusters, BPPARAM=SerialParam()){
  ...
  bluster::makeKNNGraph(x, k=min(max(2,floor(sqrt(length(unique(clusters))))-1),10),
                        BPPARAM=BPPARAM)
}
