...
.xgbtrain <- function(d2, ctype, nrounds=NULL, max_depth=6, nfold=5, eta=1,
                      ...
                      metric="logloss", ...){
  # Debug: save incoming training matrix (d2) for trace comparison
  try({
    dir.create("scdbl_traces_r", showWarnings=FALSE)
    fname <- file.path("scdbl_traces_r",
                       paste0("r_src_d2_", format(Sys.time(), "%Y%m%d%H%M%S"),
                              "_", Sys.getpid(), "_", sample(1e6,1), ".rds"))
    mat <- tryCatch(as.matrix(d2), error=function(e) d2)
    saveRDS(list(d2=mat, ctype=ctype, call=match.call()), file=fname)
    message("Saved d2 to: ", fname)
  }, silent=TRUE)

  if(!is.integer(ctype)) ctype <- as.logical(as.integer(ctype)-1L)
